...
function DashboardPage() {
  ...
  const stats = [
    {
      label: "Total Records",
      value: `${TOTAL_RECORDS_DISPLAY + (records.length - 20)}`,
      sub: `${records.length} indexed in this session`,
      icon: FileText,
    },
    {
      label: "Batches / Folders",
      value: `${Math.max(TOTAL_BATCHES_DISPLAY, batches.length)}`,
      sub: `${batches.length} active in the vault`,
      icon: Layers,
    },
    {
      ...
      <div className="space-y-8">
        ...
        <section className="vault-card flex flex-col gap-4 overflow-hidden p-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button asChild variant="outline" className="h-11 rounded-xl border-primary/30 text-primary hover:bg-primary-soft">
              <Link to="/browse">
